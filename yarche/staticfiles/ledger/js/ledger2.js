...
function colorizeCashReportTable() {
	const table = document.getElementById('cash_report-table')
	if (!table) return
	const rows = table.querySelectorAll('tbody tr')
	rows.forEach(row => {
		if (row.classList.contains('table__group-row')) return
...
const recomputeBankAccountsFromTransactions = () => {
	...
	const accountRows = bankTable.querySelectorAll(
		'tbody tr:not(.table__row--summary):not(.table__group-row)',
	)
	const baseMap = {}
	const shiftMap = {}
